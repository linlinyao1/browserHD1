...
- (void)setSelectedIndexPath:(NSIndexPath *)indexPath {
	if (![_selectedIndexPath isEqual:indexPath] || self.closing) {
		...
		if ([delegate respondsToSelector:@selector(easyTableView:selectedView:atIndexPath:deselectedView:AtIndexPath:)]) {
...
//            [self.tableView sendSubviewToBack:deselectedCell];
//            [self.tableView bringSubviewToFront:selectedCell]; // linlipeng
		}
	}
}
...
-(UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    if ([delegate respondsToSelector:@selector(easyTableView:viewForFooterInSection:)]) {
        ...
        self.sectionFooter = [self viewToHoldSectionView:sectionView];
...
{
    if ([delegate respondsToSelector:@selector(easyTableView:WillDisplayCell:AtIndex:)]) {
        [delegate easyTableView:self WillDisplayCell:cell AtIndex:indexPath];
    }
}
